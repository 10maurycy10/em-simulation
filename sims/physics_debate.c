...
// Comment out to leave circuit open
#define CLOSED 1
...
int main(int argc, char** argv) {
	...
	//Window window = window_open_file("long/");
	...
	for (int x = 0; x < w.w; x++) {
        ...
        for (int dx = -300; dx <= 300; dx++) {
                w.conductivity[center_x-dx][center_y-10] = wire;
                w.conductivity[center_x-dx][center_y+10] = wire;
        }
        ...
        // Power source gap
        w.conductivity[center_x+0][center_y+10] = 0;
	
	int exitation_x = center_x;
        int exitation_y = center_y+10;

	// Resistor
        w.conductivity[center_x-1][center_y-10] = 3;
        w.conductivity[center_x-0][center_y-10] = 3;
        w.conductivity[center_x+0][center_y-10] = 3;
        
	int probe_x = center_x;
        int probe_y = center_y-10;
	...
	while (1) {
		...
		if (t > 0.5 && w.field_e[exitation_x][exitation_y].x > -0.1) {
			w.field_j[exitation_x][exitation_y].x = 1;
		}
		printf("Current in resistor at time %f: %f\n", t, w.field_j[probe_x][probe_y].x);
