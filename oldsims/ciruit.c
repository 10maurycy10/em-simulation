...
#define ELEMENTS_H (100)
#define ELEMENTS_W (100)
...
#define ABSORBING_CONDUCTIVITY 6
...
RGB colorgrade_m(int x, int y) {
	...
	return (RGB) {
		.r = sigmoid(feild_b[x][y] * 10) * color_scale * 255,
		.g = sigmoid(total) * color_scale * 255,
		.b = sigmoid(-feild_b[x][y] * 10) * color_scale * 255
	};
}
...
RGB colorgrade_total_current(int x, int y) {
	...
	float Ey = get_e_feild(x, y).y/ELEMENTS_PITCH;
	float Ex = get_e_feild(x, y).x/ELEMENTS_PITCH;

	return (RGB) {
		.r =  sigmoid(total) * color_scale * 255,
		.g =  fabs(sigmoid(Ey/2)) * color_scale * 255,
		.b =  fabs(sigmoid(Ex/2)) * color_scale * 255,
	};
}
...
//#define COLORGRADE colorgrade_total_current
...
int main(int argc, char** argv) {
	...
	for (int dx = -10; dx <= 10; dx++) {
		feild_conductivity[center_x-dx][center_y-10] = wire;
		feild_conductivity[center_x-dx][center_y+10] = wire;
	}
	
	// Vertical wires
	for (int dy = -10; dy <= 10; dy++) {
		feild_conductivity[center_x-10][center_y-dy] = wire;
		feild_conductivity[center_x+10][center_y-dy] = wire;
	}
	
	// Capacitor gap
	feild_conductivity[center_x+10][center_y-0] = 0;
	
	// Capacitor plates
	for (int dx = 7; dx <= 13; dx++) {
		feild_conductivity[center_x+dx][center_y+1] = wire;
		feild_conductivity[center_x+dx][center_y-1] = wire;
	}
	...
	while (1) {
		...
		float current = .1;
		// Top probe
		if (t < 1) feild_j[exitation_x][exitation_y].y = current;
