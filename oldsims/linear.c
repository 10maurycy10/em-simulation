...
#define ABSORBING_THICKNESS 7
#define ABSORBING_CONDUCTIVITY 5
...
int main(int argc, char** argv) {
	...
	for (int dy = -10; dy < 10; dy++) {
		feild_conductivity[100][center_y + dy] = 100;
	}
	for (int dy = 0; dy < 85; dy++) {
		feild_conductivity[100][dy] = 100;
		feild_conductivity[100][ELEMENTS_H-dy-1] = 100;
	}
	...
	while (1) {
		...
		//if (t < (PI + 1) && t > 1) {
			for (int y = 6; y < ELEMENTS_H-6; y++) {
				feild_j[7][y].y = sin(t*5);
			}
		//}
